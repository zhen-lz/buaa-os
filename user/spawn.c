...
usr_load_elf(int fd , Elf32_Phdr *ph, int child_envid){
	...
	if (offset) {
		...
		if ((r = syscall_mem_map(child_envid, va + i, 0, (0x40000000), PTE_V | PTE_R)) < 0) {
			return r;
		}
		user_bcopy((void*)buf, (void*)((0x40000000) + offset), size);
		if ((r = syscall_mem_unmap(0, 0x40000000)) < 0) {
			return r;
		}
		i = i + size;
	}
	...
	while (i < bin_size) {
		...
		user_bcopy((void*)buf, (void*)(0x40000000), size);
		if ((r = syscall_mem_unmap(0, 0x40000000)) < 0) {
			return r;
		}
		i += size;
	...
	if (offset) {
		...
		user_bzero((void*)(0x40000000 + offset), size);
		if ((r = syscall_mem_unmap(0, 0x40000000)) < 0) {
			return r;
		}
		i = i + size;
